...
!> \param l End with event index l
!
...
!--------------------------------------------------------------------------------
!> \brief Search station list for staname, return station as output argument
!
	function searchStationNameSeismicNetwork(this,staname,station,istat) result(found)
	type (seismic_network) :: this
	character (len=*) :: staname
	logical :: found
...
!--------------------------------------------------------------------------------
!> \brief Search station list for netstaname, return station as output argument
!
	function searchNetcodeStationNameSeismicNetwork(this,netstaname,station,istat) result(found)
	type (seismic_network) :: this
	character (len=*) :: netstaname
	logical :: found
	type (seismic_station), optional :: station
	integer, optional :: istat
	integer :: idx,j
!
	idx = 0
	do j = 1, this%nstat
	    if (adjustl(trim(.netcode.(this%station(j)))//"."//trim(.staname.(this%station(j)))) &
				== adjustl(trim(netstaname))) then
	        idx = j; exit
	    endif
	enddo
	if (idx == 0) then
	...
	if(present(station)) station = this%station(idx)
	if(present(istat)) istat = idx
	end function searchNetcodeStationNameSeismicNetwork
!--------------------------------------------------------------------------------------
