...
!  its: interpolated time series restricted to overlapping range of original time series and new time point
!
...
!
!  check if new time points overlap with original time series
!
   if (tanf+(nsamp-1)*dt < this%tanf) then
      call add(errmsg,2,'new time points end before begin of time series',myname)
      return
   endif
   if (tanf > this%tanf+(this%nsamp-1)*this%dt) then
      call add(errmsg,2,'new time points start after end of time series',myname)
      return
   endif
!
!  interpolated time series gets dt of new time points
!
   its%dt = dt
!
!  check if new times start earlier than time series
!  if yes,
!     set start of interpolated time series to first new time point after begin
!     tanf+(n-1)*dt >= this%tanf ---> n >= (this%tanf-tanf)/dt+1
!  else
!     set start of its to start of new time points
!
    if (tanf < this%tanf) then
       n = ceiling((this%tanf-tanf)/dt+1.d0)
       its%tanf = tanf+(n-1)*dt
    else
       its%tanf = tanf
    endif
    if (this%exdate) then
       its%stime = this%stime.plus.(its%tanf-this%tanf)
    endif
!
!  check if new time points end later than time series
!  if yes
!     limit new time points to less than end of time series
!     its%tanf+(its%nsamp-1)*dt <= tend   --> its%nsamp <= (tend-its%tanf)/dt+1
!  else
!     its gets same number of samples as new points
!
    tend = this%tanf+(this%nsamp-1)*this%dt
    if (tanf+(nsamp-1)*dt > tend) then
       its%nsamp = floor((tend-its%tanf)/dt+1.d0)
       its%dt = dt
    else
       its%nsamp = nsamp
    endif
...
!
    do i = 1,its%nsamp
       ...
       b = 1.d0-a
       if (j == this%nsamp) then   ! j-th samle is last one, (j+1)-th sample does not exist
          its%y(i) = this%y(j)
       else
