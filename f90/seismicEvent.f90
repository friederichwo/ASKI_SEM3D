...
!------------------------------------------------------------------------
 module seismicEvent
    ...
    type seismic_event
        ...
        character (len=char_len_evid) :: evid      ! Event ID (e.g. 2006.10.2977 or 061113_141238)
        character (len=1) :: csys                          ! Character code for coordinate system (C,S)
        double precision :: slat,slon,sdepth               ! Lat and lon and depth of event (in degrees/km) or x,y,z in meters
        double precision :: mag,timeshift                  ! Magnitude of event, Time shift
        type (date_time) :: otime                          ! origin time
        double precision, dimension(:), pointer :: momten => null()    ! moment tensor (depending on istyp)
        double precision, dimension(:), pointer :: force => null()     ! force (depending on istyp)
    end type
!
 contains
!--------------------------------------------------------------------------------------
...
!
    ...
    double precision :: slat,slon,sdepth,mag,timeshift
...
!
    ...
    type (date_time) :: otime
    integer :: istyp
    double precision, dimension(:) :: momfor
!
    this%evid = evid; this%csys = csys; this%slat = slat; this%slon = slon
    this%sdepth = sdepth; this%mag = mag; this%otime = otime
    this%timeshift = timeshift
...
!
    ...
    x = cos(mc_deg2radd*(90.d0-this%slat))*cos(mc_deg2rad*this%slon)
    y = cos(mc_deg2radd*(90.d0-this%slat))*sin(mc_deg2rad*this%slon)
    z = cos(mc_deg2radd*(90.d0-this%slat))
