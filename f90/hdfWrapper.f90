...
!
    call h5fclose_f(fid,ierr)                                       ! close file
    if (ierr < 0) then
        call add(errmsg,2,'HDF file can not be opened',myname)
        return
    endif
...
!
    ...
    if (ierr < 0) goto 1
    return
 1  call add(errmsg,2,trim(filename)+' can not be opened',myname)
...
!  create a subgroup
!
    subroutine createGroupHDFWrapper(fid,groupname,grid,errmsg)
    integer(hid_t), intent(out) :: fid,grid
    character (len=*) :: groupname
    type (error_message) :: errmsg
    integer :: ierr
!
    call h5gcreate_f(fid,trim(groupname),grid,ierr)
    if (ierr < 0) then
        call add(errmsg,2,'Group '//trim(groupname)//' can not be created!','createGroupHDFWrapper')
        return
    endif
    end subroutine createGroupHDFWrapper
!---------------------------------------------------------------------
!  close a subgroup
!
...
!---------------------------------------------------------------------
!  check existence of an object by name
!  locid may stand for a file or group
!
    function existsObjectByNameHDFWrapper(locid,name,errmsg) result(ex)
    integer(hid_t), intent(out) :: locid
    character (len=*) :: name
    type (error_message) :: errmsg
    integer :: ierr
    logical :: ex
!
    call h5oexists_by_name_f(locid,trim(name),ex,ierr)
    if (ierr < 0) then
        call add(errmsg,2,'Problems with checking existence of object','existsObjectByName')
        return
    endif
    end function existsObjectByNameHDFWrapper
!---------------------------------------------------------------------
!                                                  STRING DATA    
!---------------------------------------------------------------------
